...
import { Appwrite } from '../shared/hooks/env';
import { DepaDoc } from '../shared/components/DepaDoc';

const client = new Client()
    .setEndpoint('https://cloud.appwrite.io/v1') // lonk del proyecto
    .setProject(Appwrite.projectId); //ID DEL PROYECTO
  ...
  const crearVisita = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        const promise = await databases.createDocument(
          Appwrite.databaseId, // Reemplaza con tu ID de base de datos
          Appwrite.collections.visitId, // Reemplaza con tu ID de colección
          ID.unique(),
          {
            // Los valores que sacamos del formulario
            Visitante: Visitante.value,
            Comentario: Comentario.value,
            DNI: dniNumber,
            FechaIngreso: fechaIngresoDate,
            // Los valores que sacamos del departamento
            Propietario: DepaDoc.propietario,
            Departamento: DepaDoc.departamento,
            FotoID: DepaDoc.fotoID
          }
